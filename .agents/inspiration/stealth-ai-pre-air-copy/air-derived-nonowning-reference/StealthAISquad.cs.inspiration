...
{
	public enum StealthAISquadType { Assault, Air, Rush, Protection, Naval, GeneralAttack }

	public class StealthAISquad
	{
		public List<Actor> Units = new List<Actor>();
		public StealthAISquadType Type;
		public string AirSquadDefinition;

		internal IBot Bot;
		internal World World;
		internal StealthAIModule SquadManager;
		...
		// Consecutive StealthAIIdleState scans that found no worthwhile target. This gradually increases the
		...
		public StealthAISquad(IBot bot, StealthAIModule squadManager, StealthAISquadType type)
			: this(bot, squadManager, type, null) { }

		public StealthAISquad(IBot bot, StealthAIModule squadManager, StealthAISquadType type, Actor target)
		{
			Bot = bot;
		...
		public void Update()
		{
			if (IsValid)
			{
				if (Type == StealthAISquadType.GeneralAttack)
				{
				...
				if (Type == StealthAISquadType.Air && Game.IsBenchmarking)
		...
		{
			if (IsValid && Type == StealthAISquadType.Air)
			{
		...
		public static StealthAISquad Deserialize(IBot bot, StealthAIModule squadManager, MiniYaml yaml)
		{
			var type = StealthAISquadType.Rush;
			Actor targetActor = null;

			var typeNode = yaml.Nodes.FirstOrDefault(n => n.Key == "Type");
			if (typeNode != null)
				type = FieldLoader.GetValue<StealthAISquadType>("Type", typeNode.Value.Value);
			...
			var squad = new StealthAISquad(bot, squadManager, type, targetActor);
			...
			if (squad.Type == StealthAISquadType.GeneralAttack)
