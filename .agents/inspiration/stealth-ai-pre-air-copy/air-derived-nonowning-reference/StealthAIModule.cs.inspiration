...
{
	enum StealthAIGeneralGroundMissionStatus
	{
		MissingSquad,
		InvalidTarget,
		Urgent,
	...
	public class StealthAIModuleInfo : ConditionalTraitInfo
	{
		...
		public override object Create(ActorInitializer init) { return new StealthAIModule(init.Self, this); }
	}

	public class StealthAIModule : ConditionalTrait<StealthAIModuleInfo>, IBotEnabled, IBotTick, IBotRespondToAttack,
		IBotPositionsUpdated, INotifyKilled, INotifyAppliedDamage, IGameSaveTraitData, IAdvancedBotTick
	{
		...
		public List<StealthAISquad> Squads = new List<StealthAISquad>();
		...
		public StealthAIModule(Actor self, StealthAIModuleInfo info)
			: base(info)
		{
			World = self.World;
			Player = self.Owner;
		...
		string IAdvancedBotTick.FailsafeModuleId => "StealthAIModule";
		...
		void IAdvancedBotTick.SetAdvancedBehaviorEnabled(bool enabled)
		{
			...
			if (enabled && Info.GroundTargetDebugLogging)
			{
				...
				if (retained.Length > 0)
					Log.Write("debug", "StealthAISquad failsafe handoff [{0}]: owner=StealthAIModule state=ordinary-manager " +
		...
		internal void RetainFailsafeReleasedActors(string source, IEnumerable<Actor> actors)
		{
			...
			if (Info.GroundTargetDebugLogging && released.Length > 0)
				Log.Write("debug", "StealthAISquad failsafe handoff [{0}]: owner=StealthAIModule source={1} retained={2} " +
		...
		void CleanSquads()
		{
			...
			foreach (var s in Squads)
			{
				...
				if (s.Type == StealthAISquadType.Air)
					s.CleanAirMembership();
				else if (s.Type == StealthAISquadType.GeneralAttack)
		...
		{
			return actor != null && Squads.Any(s => s.Type == StealthAISquadType.Protection && s.Units.Contains(actor));
		}
			...
			out WPos formationCenter, out CPos objective, out bool urgent)
		{
			...
			urgent = status == StealthAIGeneralGroundMissionStatus.Urgent;
			return status == StealthAIGeneralGroundMissionStatus.Ready || urgent;
		}

		internal StealthAIGeneralGroundMissionStatus GetGeneralGroundMissionStatus(Actor actor, out Actor target,
			out WPos formationCenter, out CPos objective)
		{
			var squad = Squads.FirstOrDefault(s => s.Type == StealthAISquadType.GeneralAttack && s.Units.Contains(actor));
			if (squad == null)
			{
				target = null;
				formationCenter = WPos.Zero;
				objective = CPos.Zero;
				return StealthAIGeneralGroundMissionStatus.MissingSquad;
			}
			...
			if (!squad.IsTargetValid)
			{
				target = null;
				objective = CPos.Zero;
				return StealthAIGeneralGroundMissionStatus.InvalidTarget;
			}
			...
			return IsUnitProtectingBase(actor) || squad.FuzzyStateMachine.Is<GroundUnitsFleeState>() ?
				StealthAIGeneralGroundMissionStatus.Urgent : StealthAIGeneralGroundMissionStatus.Ready;
		}

		internal CPos GroundRecoveryDestination()
		{
			var squad = Squads.FirstOrDefault(s => s.Type == StealthAISquadType.GeneralAttack);
		...
		StealthAISquad GetSquadOfType(StealthAISquadType type)
		{
		...
		/// </summary>
		StealthAISquad GetAirSquadWithRoom(IBot bot, Actor actor)
		{
			if (Info.AirSquadDefinitions.Count > 0)
			{
				...
				var squads = Squads.Where(s => s.Type == StealthAISquadType.Air && s.AirSquadDefinition == selected.Key).ToList();
				if (squads.Count < selected.Value.SquadCount)
				{
					var created = RegisterNewSquad(bot, StealthAISquadType.Air);
			...
			return RegisterNewSquad(bot, StealthAISquadType.Air);
		}

		StealthAISquad RegisterNewSquad(IBot bot, StealthAISquadType type, Actor target = null)
		{
			var ret = new StealthAISquad(bot, this, type, target);
			Squads.Add(ret);
			return ret;
		}
		...
		internal int AdoptTransportedAssault(IBot bot, IEnumerable<Actor> transportedUnits, Actor preferredTarget)
		{
			...
			var assault = RegisterNewSquad(bot, StealthAISquadType.Assault, target);
		...
		void AssignRolesToIdleUnitsDegraded(IBot bot)
		{
			...
			if (--attackForceTicks <= 0)
			{
				attackForceTicks = Info.AttackForceInterval;
				foreach (var squad in Squads)
					if (squad.Type != StealthAISquadType.GeneralAttack && squad.Type != StealthAISquadType.Air)
						squad.Update();
			}
			...
			if (Info.GroundTargetDebugLogging)
				foreach (var group in retainedGroups.Where(g => g.Value.Length > 0))
				{
					...
					Log.Write("debug", "StealthAISquad failsafe released control [{0}]: source={1} fallback={2} protection={3} " +
		...
		void RecruitUnassignedCombatUnits(IBot bot)
		{
			...
			if (Info.GroundTargetDebugLogging && adopted.Count <= 32)
				Log.Write("debug", "StealthAISquad registry recruitment [{0}]: owner=StealthAIModule advanced={1} " +
		...
		bool AdoptCurrentSquadUnit(IBot bot, Actor actor, List<Actor> adoptedGroundActors)
		{
			...
			{
				var ships = GetSquadOfType(StealthAISquadType.Naval) ?? RegisterNewSquad(bot, StealthAISquadType.Naval);
				ships.Units.Add(actor);
			}
			else if (Info.UseCohesiveGroundSquad)
			{
				adoptedGroundActors.Add(actor);
				var ground = GetSquadOfType(StealthAISquadType.GeneralAttack);
				if (ground == null)
			...
			{
				adoptedGroundActors.Add(actor);
				var assault = Squads.FirstOrDefault(s => s.Type == StealthAISquadType.Assault && s.IsValid);
				var target = FindClosestEnemy(actor.CenterPosition);
				if (assault == null)
					assault = RegisterNewSquad(bot, StealthAISquadType.Assault, target);
				else if (target != null)
				...
				if (target != null)
				{
					...
					assault.FuzzyStateMachine.ChangeState(assault, new GroundUnitsAttackMoveState(), true);
					if (Info.GroundTargetDebugLogging)
						Log.Write("debug", "StealthAISquad immediate pre-Codex adoption [{0}]: actor={1}#{2} " +
		...
		void AdoptLegacyFallbackAssault(IBot bot, List<Actor> actors, List<Actor> adopted)
		{
			...
			assault.FuzzyStateMachine.ChangeState(assault, new GroundUnitsAttackMoveState(), true);
			if (Info.GroundTargetDebugLogging)
				Log.Write("debug", "StealthAISquad registry fallback [{0}]: owner=StealthAIModule source=pre-codex-assault " +
		...
		void QueueFailsafeFallback(IBot bot, string source, IEnumerable<Actor> candidates)
		{
			...
			if (Info.GroundTargetDebugLogging)
				Log.Write("debug", "StealthAISquad failsafe fallback [{0}]: owner=StealthAIModule source={1} accepted={2} " +
		...
		void FindNewUnits(IBot bot)
		{
			...
			var assignedAircraft = new HashSet<Actor>(Squads.Where(s => s.Type == StealthAISquadType.Air).SelectMany(s => s.Units));
			...
			if (Info.GroundTargetDebugLogging && adoptedGroundActors.Count > 0 && adoptedGroundActors.Count <= 32)
				Log.Write("debug", "StealthAISquad ordinary adoption [{0}]: owner=StealthAIModule actors={1}.",
		...
		void CreateAttackForce(IBot bot)
		{
			...
			{
				var attackForce = Info.UseCohesiveGroundSquad ? GetSquadOfType(StealthAISquadType.GeneralAttack) : null;
				if (attackForce == null)
					attackForce = RegisterNewSquad(bot,
						Info.UseCohesiveGroundSquad ? StealthAISquadType.GeneralAttack : StealthAISquadType.Assault);
		...
		void TryToRushAttack(IBot bot)
		{
			...
			foreach (var b in allEnemyBaseBuilder)
			{
				...
				if (AttackOrFleeFuzzy.Rush.CanAttack(ownUnits, enemies))
				{
					...
					var rush = GetSquadOfType(StealthAISquadType.Rush);
					if (rush == null)
						rush = RegisterNewSquad(bot, StealthAISquadType.Rush, target);
		...
		{
			var protectSq = GetSquadOfType(StealthAISquadType.Protection);
			if (protectSq == null)
				protectSq = RegisterNewSquad(bot, StealthAISquadType.Protection, attacker);
			...
			if (!protectSq.IsValid)
			{
				...
				foreach (var a in ownUnits)
				{
					...
					foreach (var ground in Squads.Where(s => s.Type == StealthAISquadType.GeneralAttack && s.Units.Contains(a)))
				...
				if (Info.GroundTargetDebugLogging && ownUnits.Count > 0)
					Log.Write("debug", "Ground protection [{0}] activated against {1}#{2}: defenders={3} general-shared={4}.",
						...
						ownUnits.Count(a => Squads.Any(s => s.Type == StealthAISquadType.GeneralAttack && s.Units.Contains(a))));
			}
		}
		...
		string AirProfileFor(Actor actor)
		{
			...
			var squad = Squads.FirstOrDefault(s => s.Type == StealthAISquadType.Air && s.Units.Contains(actor));
			if (squad != null)
		...
		void INotifyAppliedDamage.AppliedDamage(Actor self, Actor damaged, AttackInfo e)
		{
			...
			if (Info.GroundTargetDebugLogging)
			{
				var groundSquad = Squads.FirstOrDefault(s => s.Type == StealthAISquadType.GeneralAttack &&
				...
				var protectionSquad = Squads.FirstOrDefault(s => s.Type == StealthAISquadType.Protection && s.Units.Contains(e.Attacker));
		...
		List<MiniYamlNode> IGameSaveTraitData.IssueTraitData(Actor self)
		{
			...
			{
				new MiniYamlNode("Squads", "", Squads.Select(s => new MiniYamlNode("StealthAISquad", s.Serialize())).ToList()),
		...
		void IGameSaveTraitData.ResolveTraitData(Actor self, List<MiniYamlNode> data)
		{
			...
			if (squadsNode != null)
			{
				...
				foreach (var n in squadsNode.Value.Nodes)
					Squads.Add(StealthAISquad.Deserialize(bot, this, n.Value));
			}
		}
	}
}
