	...
	public class Move : Activity
	{
		...
		public Move(Actor self, CPos destination, Color? targetLineColor = null)
		{
			...
			getPath = check =>
			{
				...
				using (var search = PathSearch.ToTargetCell(
					...
					return (false, mobile.Pathfinder.FindPath(search));
			};
			...
			Color? targetLineColor = null)
		{
			...
			getPath = check =>
			{
				...
				return (false, mobile.Pathfinder.FindUnitPath(mobile.ToCell, this.destination.Value, self, ignoreActor, check));
			};
		...
		public Move(Actor self, Func<BlockedByActor, (bool AlreadyAtDestination, List<CPos> Path)> getPath,
			Color? targetLineColor = null, Actor ignoreActor = null)
		{
			// PERF: Because we can be sure that OccupiesSpace is Mobile here, we can save some performance by avoiding querying for the trait.
			mobile = (Mobile)self.OccupiesSpace;
		...
		public override bool Tick(Actor self)
		{
			...
			{
				// We cannot reach our destination, but can still move aside to unblock other units.
