...
- Match Commenters read assigned game artifacts and optionally `.agents/references/LIBERTY-DAWN-DESIGN.md`; they do not read implementation context. Policy Reviewers read only that design document, short task context, their assigned factual or proposed-policy narrative, and a staged copy of the bounded persistent policy scratchpad, apart from role instructions and a path-only job envelope. Stealth-tank policy jobs additionally receive `.agents/references/STEALTH-TANK-LIFECYCLE.md`.
