...
- Status: `First iteration - testing; core armed-support rule proved, final active-engagement repair needs engine proof`
...
- Current cycle: `8`; cycles used: `5/5 primary`, `3 exceptional Sol-medium acceptance cycles`
...
- Proposed status: `First iteration - testing; core armed-support rule proved, final active-engagement repair unplayed`
- Branch/head and PR/checks: `agent/round-20260813-cnc96a-air-squad-stealth` / cycle-8 commit; PR `none`; strict Release build/full CNC MiniYAML 0 warnings/errors; focused specialist policy suite 67/67; both cycle-8 maps lint; `git diff --check` clean.
- Cycles/models used: `cycle 1 Sol high, cycle 2 Terra medium, cycles 3-4 explicitly authorized Sol high, cycle 5 Terra medium, cycles 6-8 explicitly authorized exceptional Sol medium`
...
- Per-game Luna narrative/policy paths: `cycle8/reviews/game-a-luna-narrator/NARRATIVE.md and game-a-luna-policy/POLICY-REVIEW.md record exact combined coverage and same-call resume failure. cycle8/reviews/game-b-luna-narrator/NARRATIVE.md and game-b-luna-policy/POLICY-REVIEW.md record lone-MHQ continued damage, combined Stop, and missing recovery.`
- User policy-review decision and next boundary: `Cycle 8 implemented the corrected detector doctrine. The user then directed cycle 9 toward Air-shaped safe-primary -> weakest anti-stealth blocker -> reassess, including safely isolated unarmed MHQ clearing, plus flee/repair/rejoin lifecycle. Cycle 8 stops without that broader policy.`
- Cycle-8 review/disposition: `Both Luna narratives/reviews were fact-checked. Game A called the intent sensible and same-call transition a high-priority failure; its idempotence guard was adopted. Game B found the core rule demonstrated and recovery a medium-high evidence gap; active-assignment preservation was adopted. Generation telemetry and broader lifecycle work are deferred.`
...
- Diagnostic/performance result: `Games completed in 6.008/6.010s at ticks 2100/1700. Game A final tick times were 0.47/0.41/0.33ms; Game B 0.34/0.45/0.43ms. Startup dominated worst ticks (~1009/1000ms). Strategy remained 75 ticks, engagement safety 25 ticks, grid 4 cells, cache 125 ticks; no Air identity appeared.`
- Deferred work and known risks: `Final active-engagement retention/recovery correction has focused but no engine proof. Cycle 9 must map AirSquad safe-primary -> weakest-blocker -> reassess, flee, repair, and rejoin seams and safely allow an isolated unarmed MHQ clear without weakening armed coverage. Repair is opportunistic exactly like Air: without compatible reachable repair, damaged specialists remain active with bounded hit-run/flee until death, never park/idle/leave/wait; reevaluate later repair and test both no-repair active and repair/rejoin. Explicit SnipeTank/Crush attribution, remaining personalities, blocked/reservation cases, controls, paced agreement, save/load, final regression, PR/CI, and cleanup remain.`
