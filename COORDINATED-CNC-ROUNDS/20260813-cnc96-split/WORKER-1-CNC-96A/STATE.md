...
- Current cycle: `1`; cycles used: `1/5 primary`, `0/10 optional Luna`
...
- Games completed: `2 distinct changed-build scenarios plus matched exact-base controls`;
...
- Branch/head and PR/checks: `agent/round-20260813-cnc96a-stealth-performance` / cycle-1 receipt commit; PR `none`; protected `make all` 0 warnings/errors; focused `StealthTankSquadPolicyTest` 42/42; `git diff --check` clean.
- Cycles/models used: `1 Sol-high primary cycle; no optional/escalation cycles`
- Acceptance/adversarial/final-regression evidence: `Scenario A materially reduced directly attributed Stealth work and aligned tick tails while Stealth/Chemical/Air all acted. Scenario B cleanly exercised scripted state changes with Chemical/Air activity and no stale Stealth orders, but failed to prove Stealth recovery or useful progress after the late opening. Only IronReaper/paced mode and no explicit specialist damage/loss telemetry: not final acceptance.`
- Old-control and direct AirSquad comparative result: `Exact-base 0c9a5c1 matched both maps/seeds/options. Scenario A combined Stealth CPU -76.5%, orders -75.7%, tick p99 -52.7%, >=50ms ticks -30.8%; Chemical CPU -16.7%, orders -81.8%. Scenario B Stealth CPU -3.6% with zero orders on both legs; tick p99 29 -> 32ms. AirSquad showed stable bounded state/target/order behavior, but per-Air CPU is unavailable and aggregate SquadManager is not claimed as direct Air attribution.`
- Per-game Luna narrative paths/conclusions: `Scenario A: analysis/20260813-cnc96-split/worker-1-cnc-96a/commenters/scenario-a/NARRATIVE.md — both specialist profiles and Air acted; repeated specialist paths/orders vanished; strong direct CPU/tail improvement; combat outcome unknown. Scenario B: analysis/20260813-cnc96-split/worker-1-cnc-96a/commenters/scenario-b/NARRATIVE.md — all scripted transitions/final counts passed; Chemical and Air acted; Stealth held throughout and recovery/reachability is unknown.`
- User manual policy-review decision/path and next authorization: `Pending; no automated Match Policy Reviewer was launched. Ask: (1) approve safe/progressing plan retention with target/member/move/route-safety/no-progress invalidations at the unchanged 75-tick scan; (2) classify Scenario B Stealth zero-order hold as acceptable safety or passive idling requiring a guaranteed-reachable recovery test; (3) decide whether explicit damage/survival and direct-Air attribution must precede any cycle-2 code change. Do not start cycle 2 until the coordinator records the user's decision.`
- Cycle-3 code review/disposition: `Not due in cycle 1.`
...
- Diagnostic/performance result: `Default-off aggregate per-scan lines proved one factual-view build plus one same-tick hit per player; stable Scenario A scans retained 2 Stealth + 1 Chemical plans with zero replacement paths/orders. Exact-base retained periodic Stealth tails. Scenario A ordinary SquadManager CPU +6.8% combined and orders 1000 -> 1921 under divergent downstream combat, so repeated direct-Air controls remain required.`
- Deferred work and known risks: `Explicit target damage/HP/loss/arrival markers; direct Air timing; guaranteed reachable reopening; pending explosion; transport/crate reservation handoff; other nine bots; repeated controls; headless/paced agreement; save/load applicability; final regression and PR/CI remain. Historical 8024fd2 is incompatible with the modern IronReaper/launcher matrix and was not relabelled or backported.`
