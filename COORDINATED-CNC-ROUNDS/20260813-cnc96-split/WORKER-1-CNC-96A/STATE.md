...
- Current cycle: `2`; cycles used: `2/5 primary`, `0/10 optional Luna`
...
- Games completed: `cycle 1: 2 distinct changed-build scenarios plus matched exact-base controls; cycle 2: 2 distinct changed-build scenarios`;
  cycle-3 review: `not due`; PR: `none`
...
- Branch/head and PR/checks: `agent/round-20260813-cnc96a-stealth-performance` / cycle-2 receipt commit; PR `none`; protected CNC compile/lint 0 warnings/errors; focused `StealthTankSquadPolicyTest` 43/43; `git diff --check` clean.
- Cycles/models used: `1 Sol-high plus 1 Terra-medium primary cycle; no optional/escalation cycles`
- Acceptance/adversarial/final-regression evidence: `The reachable scenario resolves cycle 1's zero-order fixture ambiguity: both AIs issued real routes and attacks at the first scan after targets existed, caused sampled damage, destroyed opening and late targets, recovered after removal/member replacement, and retained specialist counts. Saturation directly compared Air and specialist cadence/cost. Other bot personalities, blocked/reservation/hazard edges, repeated controls, paced final regression, and PR/CI remain.`
- Old-control and direct AirSquad comparative result: `Cycle 1 exact-base comparison remains the old-policy evidence: combined Stealth CPU -76.5%, orders -75.7%, tick p99 -52.7%. Cycle 2 directly attributed Air at the changed head. Over 900 ticks, the two Stealth profile instances together made 24 substantive scans but their performance wrapper recorded 1,800 per-tick invocations and 7790.150ms; SkyNet's four Air squads made 72 strategic calls and 144 safety calls, while IronReaper's one Generic Air squad made 12 strategic calls. These are not equal denominators and are normalized in the report.`
- Per-game Luna narrative paths/conclusions: `Saturation: analysis/20260813-cnc96-split/worker-1-cnc-96a/commenters/cycle2-saturation/NARRATIVE.md — both specialists/Air acted, stable plans avoided refresh churn, and late all-dangerous holds were visible. Reachable: analysis/20260813-cnc96-split/worker-1-cnc-96a/commenters/cycle2-reachable/NARRATIVE.md — both sides routed to, damaged, and destroyed guaranteed hostile opening/late targets and recovered after scripted transitions.`
- User manual policy-review decision/path and next authorization: `User approved retaining a safe progressing plan until target/member/movement/route-safety/no-progress invalidation, rejected cycle 1's zero-order run as acceptance evidence, and required guaranteed actionable openings plus direct Air CPU/order evidence. Those requirements are now met. Ask whether cycle 3 may narrowly reduce the remaining expensive Stealth initial/replan hazard path work without changing targets, safety, cadence, group/candidate bounds, or profile code sharing; recommend retaining the current 75-tick full safety/replan boundary rather than adding a new 25-tick policy.`
- Cycle-3 code review/disposition: `Not due yet; required after the next authorized cycle.`
...
- Diagnostic/performance result: `Per-tick specialist dispatch is now explicitly tested as an O(1) countdown; full work ran only at ticks 1,76,...826. Stealth's opening/replan exact ground paths still dominate: saturation Stealth 7790.150ms versus all directly measured Air strategy 1908.601ms and Air local safety 16.383ms. Chemical's shared-view hit path was 67.161ms. Reachable Stealth was only 211.994ms over 20 substantive profile scans, showing geometry/path workload drives the saturation spike.`
- Deferred work and known risks: `The first/replan hazard-aware route still performs one exact ground path per assigned unit and tests visited cells against resource and threat state; this is the main measured remaining cost. Other bot personalities, blocked topology, pending explosion, transport/crate reservation handoff, repeated controls, paced/headless agreement, save/load applicability, final regression, PR/CI, and cleanup of diagnostic-only Air attribution remain. Historical 8024fd2 remains incompatible and was not relabelled or backported.`
