...
## Final Terra review response (one authorized cycle)

- Accepted the final-review finding. The manager now persists `repairIntentPending` as soon as the active cluster has its role-ready core and a protected local repair site, before any Building queue accepts `fix`. The intent is serialized (with backward read support for the prior recovery field), remains after placement is ordered, and clears only when a qualifying live local facility exists or the anchor is validly invalidated. A repair reservation lost with its producer starts one new bounded handoff episode, so a handoff used by the destroyed producer cannot strand the intent.
- Focused policy test added for the pre-reservation intent boundary; focused suite passed 13/13 and full suite passed 531/531. `git diff --check` passed.
- Fresh R7 changed/control pair used the existing byte-matched producer-loss fixture and reached tick 8,500 cleanly with no fatal, desync, Lua, rules, or unhandled error. Control passed. Changed established intent at tick 1,501 while its Building queue was occupied, deferred correctly for opening, handed `fix` behind one ordinary item on `fact#10@20,30` at tick 2,078, placed `fix@53,28`, and observed a qualifying live local repair plus cluster completion at tick 3,126. The fixture destroyed `fact@23,24`, not the selected producer, so the changed manifest intentionally failed its producer-loss-only patterns; this run proves pre-reservation contention and live repair, not a new end-to-end producer-loss recovery.
- The final observer at tick 8,301 had three local role-covering towers and one live local/global `fix@53,28`, but no completed open screen. Therefore this response does not satisfy the review's full release portfolio and status remains `First iteration - testing`; required deterministic producer-loss, screen, three adversarial, persistence, topology, natural-match, and final-regression evidence remains for integration.
- Evidence: `AUTONOMOUS-CNC-LOGS/20260807-bug-polish-03/WORKER-2-CNC-46/final-terra-repair-pre-reservation/results-changed/batch-summary.json`, `.../results-changed/changed-cnc46-final-terra-repair/support/Logs/debug.log`, and `.../results-control/batch-summary.json`.
