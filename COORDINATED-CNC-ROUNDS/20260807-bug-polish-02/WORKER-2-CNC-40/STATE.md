...
- Full-engine game tests completed: `10` (one base probe, one invalid changed harness run with material outcome evidence, three valid changed runs, three matched base controls, and a changed/control natural-conclusion pair invalid only for ending before its configured duration; excluded two tick-0 map-bootstrap failures without game evidence)
