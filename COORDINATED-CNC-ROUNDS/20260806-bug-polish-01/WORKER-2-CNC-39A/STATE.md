...
| 11 | Review-response commit `f3fbbb4da48a66739bfc7195a3f3b4f91e5e3d16`: serialize and deterministically validate/restore both scan timers, capture/demolition assignments, claimant cardinality, progress history, and deferred-target retries | A save during paid travel may erase the incumbent reservation and allow the opposite purpose to claim the same target after reload. Two seed-39085 directions saved at tick 25: active Engineer with idle Commando, then active Commando with idle Engineer pair; transfer the incumbent at tick 151 and require identical release/reselection/outcome | Focused 11/11; full unit 445/445; `make all`, `make check`, `make check-scripts`, `make test`, and `git diff --check` passed. Fresh cycle-11 combined rerun passed to tick 2400. Final fresh and reload pairs all passed; full-engine counter 65 | Factual review `cycle-11/persistence-commenter-2/NARRATIVE.md` confirmed exact fresh/reload transition ticks and outcomes. Policy `cycle-11/persistence-policy-2/POLICY-REVIEW.md`: **APPROVE**. Earlier fixture-tuning and superseded tick-5 runs are excluded from acceptance | Capture reload restored `e6#154 -> weap#156` at tick 27 with assigned tick 1/last progress 25; Commando stayed idle through tick 101, took the released target at tick 152, and destroyed it at 1496 in both runs. Demolition reload restored `rmbo#155 -> weap#156` with the same history; Engineer stayed idle through 101, the exact pair took the released target at 152, and captured it at 301 in both runs. No fatal/desync/Lua exception | Required persistence defect resolved in the single allowed review-response cycle; push the narrow correction to PR #80 while retaining First iteration - testing for the unchanged defender/natural-production/transport evidence gaps |
...
- Match narratives and routine policy-review conclusions: fresh Terra factual/policy outputs for every judged batch under `analysis/worker-2-cnc-39a/`; final persistence outputs `cycle-11/persistence-commenter-2/NARRATIVE.md` and `cycle-11/persistence-policy-2/POLICY-REVIEW.md` confirm exact restored incumbents/outcomes and **APPROVE** the policy
- Sol-xhigh policy escalation (unused, or test count/path/conclusion): `unused`
- Final regression: `cycle-11/fresh-combined-3/combined-final` passed to tick 2400 at about 239.791 world ticks/sec; exact Engineer pair captured the shared target, Commando destroyed the alternate, allied race canceled before plant, hostile follow-up completed
- Error/warning and diagnostic-cleanup result: focused 11/11; full unit 445/445; `make all`, `make check`, `make check-scripts`, `make test`, and `git diff --check` passed; 0 final build warnings/errors; no fatal/desync; transition and one-time restore diagnostics are BotDebug-gated
- Performance/determinism result: stable target ActorIDs/purpose order/outcomes across repeats and reloads; final fresh persistence pair ~337.01 aggregate world ticks/sec versus reload pair ~336.883; exact final transition ticks matched in both directions; no repeatable >5% regression
...
- Relevant artifact paths: task `REPORT.md`; `analysis/worker-2-cnc-39a/cycle-3/`, `cycle-5/`, `cycle-7/`, `cycle-9/`, `cycle-10/`, and `cycle-11/`
...
- Test the exact cumulative candidate fd15540ffc98c70f085688fe0b38a4a6341fc6ed
  ...
  from repair branch agent/round-20260806-cnc39a-rc2-repair.
