...
def phase_results(batch: Path) -> dict[str, Any]:
    ...
    for run in summary["runs"]:
        ...
        game_seconds = run["maximum_engine_world_tick"] * TICK_SECONDS
...
def write_markdown(path: Path, result: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        "",
        f"- Map: Empire Earth4; fixed horizon: {result['ticks']} ticks.",
        ...
        "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
        f"| Simple AttackMove baseline | {result['runs']} | {baseline['game_seconds']:.3f} | "
        ...
        f"{baseline['module_cpu_seconds']:.3f} |",
        f"| Full modules | {result['runs']} | {full['game_seconds']:.3f} | "
        f"{full['wall_seconds']:.3f} | {full['game_seconds_per_wall_second']:.3f}x | "
        f"{full['cpu_seconds']:.3f} | {full['tick_mean_milliseconds']:.3f} | "
        f"{full['module_cpu_seconds']:.3f} |",
        "",
        f"Full-module wall-time change: {delta['wall_time_change_percent']:+.2f}%. "
        f"CPU-time change: {delta['cpu_time_change_percent']:+.2f}%. "
        f"Mean-tick-time change: {delta['mean_tick_time_change_percent']:+.2f}%. "
        f"Bot-module CPU-time change: {delta['module_cpu_time_change_percent']:+.2f}%.",
        "",
...
def main() -> int:
    args = parse_args()
    if not 1 <= args.runs <= 20 or args.ticks < 1 or args.timeout <= 0:
        raise SystemExit("runs must be 1..20; ticks and timeout must be positive")
    ...
    manifests = {
        "baseline": phase_manifest(
            "baseline", baseline_map, args.runs, args.ticks, args.seed, args.timeout, True,
        ),
        "full": phase_manifest(
            "full", args.map.resolve(), args.runs, args.ticks, args.seed, args.timeout, False,
        ),
    }
