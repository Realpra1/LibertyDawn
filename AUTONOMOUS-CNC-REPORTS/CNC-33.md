...
- Follow-up CNC-33A owns a likely refinery-throughput regression exposed after publication: the all-refineries-occupied/750-tick trigger and single global reservation can react too slowly with several idle facts, while the current cash-reservation flag can pause unit production even when a refinery is already affordable. The next task must use bounded harvester/refinery capacity plus congestion, exploit multiple idle facts without recreating refinery spam, account for the refinery's free ordinary/stealth harvester and resulting vehicle-queue capacity, and reserve only the real cash shortfall. Its required VIKI/Skynet differential gates are recorded in the task sheet.
